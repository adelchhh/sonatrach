...
export default function AuditLogPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return logs.filter((l) => {
      ...
      if (q) {
        const hay = [l.action, l.target_name, l.user_first_name, l.user_last_name, l.employee_number, l.ip_address]
          .filter(Boolean)
          .join(" ")
          .toLowerCase();
            ...
            <div className="space-y-6">
              <div>
                <h1 className="text-[36px] font-extrabold text-[#2F343B] leading-[110%]">
                  {t("system.auditLog.title")}
                </h1>
                <p className="text-[#7A8088] text-sm mt-2 max-w-[760px] leading-[170%]">
                  {t("system.auditLog.subtitle")}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
                <StatCard title={t("system.auditLog.statEntries")} value={logs.length} />
                <StatCard title={t("system.auditLog.statActions")} value={actions.length} />
                <StatCard title={t("system.auditLog.statTables")} value={targets.length} />
                <StatCard
                  title={t("system.auditLog.statLast")}
              ...
              <section className="rounded-[24px] bg-white border border-[#E5E2DC] p-5">
                <div className="flex flex-wrap gap-3">
                  <input
                    ...
                    placeholder={t("system.auditLog.filters.search")}
                  ...
                  >
                    <option value="all">{t("system.auditLog.filters.allActions")}</option>
                    {actions.map((a) => (
                  ...
                  <select
                    ...
                    className="px-4 py-3 rounded-[14px] border border-[#E5E2DC] bg-[#F7F7F5] text-sm outline-none"
                  >
                    <option value="all">{t("system.auditLog.filters.allTables")}</option>
                    {targets.map((tg) => (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[1100px]">
                    <thead className="bg-[#FBFAF8]">
                      <tr>
                        <th className="px-5 py-4 text-left text-xs font-semibold text-[#7A8088] uppercase">{t("system.auditLog.col.date")}</th>
                        <th className="px-5 py-4 text-left text-xs font-semibold text-[#7A8088] uppercase">{t("system.auditLog.col.user")}</th>
                        <th className="px-5 py-4 text-left text-xs font-semibold text-[#7A8088] uppercase">{t("system.auditLog.col.action")}</th>
                        <th className="px-5 py-4 text-left text-xs font-semibold text-[#7A8088] uppercase">{t("system.auditLog.col.target")}</th>
                        <th className="px-5 py-4 text-left text-xs font-semibold text-[#7A8088] uppercase">{t("system.auditLog.col.ip")}</th>
                        <th className="px-5 py-4 text-left text-xs font-semibold text-[#7A8088] uppercase">{t("system.auditLog.col.details")}</th>
                      </tr>
                    </thead>
                    <tbody>
                      {loading && (
                        <tr>
                          <td colSpan="6" className="px-5 py-10 text-center text-sm text-[#7A8088]">
                        ...
                        <tr key={l.id} className="border-t border-[#E5E2DC]">
                          ...
                          <td className="px-5 py-4 text-sm text-[#2F343B]">
                            {l.user_first_name || l.user_last_name ? (
                              <>
                                <p className="font-semibold">{l.user_first_name} {l.user_last_name}</p>
                                <p className="text-xs text-[#7A8088]">{l.employee_number}</p>
                              </>
                            ) : (
                              <span className="text-[#7A8088] italic">{t("system.auditLog.system")}</span>
                            )}
                          </td>
                          <td className="px-5 py-4">
                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${actionStyle(l.action)}`}>
                              {l.action}
                            </span>
                          </td>
                          <td className="px-5 py-4 text-sm text-[#2F343B]">
                            {l.target_name || <span className="text-[#7A8088] italic">—</span>}
                            {l.target_table && (
                              <p className="text-xs text-[#7A8088] mt-1">
                                {l.target_table}{l.target_id ? ` #${l.target_id}` : ""}
                              </p>
                            )}
                          </td>
        ...
        >
          ...
          >
            <h2 className="text-xl font-bold text-[#2F343B] mb-3">
              {t("system.auditLog.detailsTitle")}
            </h2>
