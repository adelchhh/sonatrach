...
export default function ManageSystemAdmins() {
  ...
  const loadAdmins = async () => {
    try {
      ...
      console.error(err);
      setError(t("system.manageSystemAdmins.errorLoad"));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAssign = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/system/users/${foundEmployee.id}/roles/system-admin`, { method: "POST" });
      const data = await res.json();
      if (!res.ok) {
        setError(data.message || t("system.manageSystemAdmins.errorAssign"));
        return;
      }
      setFoundEmployee(null);
      setSearchValue("");
      ...
      console.error(err);
      setError(t("system.manageSystemAdmins.errorAssign"));
    }
  };

  const handleRemove = async (id) => {
    try {
      const res = await fetch(`${API_URL}/system/users/${id}/roles/system-admin`, { method: "DELETE" });
      const data = await res.json();
      if (!res.ok) {
        setError(data.message || t("system.manageSystemAdmins.errorRemove"));
        return;
      }
      await loadAdmins();
    } catch (err) {
      console.error(err);
      setError(t("system.manageSystemAdmins.errorRemove"));
    }
  };
        ...
        <main className="flex-1 overflow-y-auto p-6 space-y-6">
          <div>
            ...
            <h1 className="text-[36px] font-extrabold text-[#2F343B]">
              {t("system.manageSystemAdmins.title")}
            </h1>
            <p className="text-[#7A8088] text-sm mt-2">
              {t("system.manageSystemAdmins.subtitle")}
            </p>
          </div>

          <div className="rounded-[14px] border border-yellow-300 bg-yellow-50 text-yellow-800 px-4 py-3 text-sm">
            ⚠️ {t("system.manageSystemAdmins.warning")}
          </div>

          <section className="rounded-[24px] bg-white border border-[#E5E2DC] p-5">
            <h2 className="text-[22px] font-bold text-[#2F343B] mb-4">
              {t("system.manageSystemAdmins.assignTitle")}
            </h2>
            ...
            <div className="flex gap-3">
              <input
                ...
                placeholder={t("system.manageSystemAdmins.searchPlaceholder")}
          ...
          <section className="rounded-[24px] bg-white border border-[#E5E2DC] overflow-hidden">
            <div className="px-5 py-4 border-b">
              <h2 className="font-bold text-lg text-[#2F343B]">
                {t("system.manageSystemAdmins.currentTitle")}
              </h2>
            </div>
                  ...
                  {admins.length === 0 && (
                    <tr>
                      <td colSpan="5" className="text-center py-6 text-gray-400">
                        {t("system.manageSystemAdmins.emptyText")}
                      </td>
                    </tr>
                  )}
                </tbody>
