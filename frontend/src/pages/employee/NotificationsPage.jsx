        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="space-y-6 max-w-[900px]">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-[#7A8088] text-sm mt-2 max-w-[760px] leading-[170%]">
                  {t("employee.notifications.subtitle")}
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <StatCard title={t("employee.notifications.statTotal")} value={stats.total} />
              <StatCard title={t("employee.notifications.statUnread")} value={stats.unread} />
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <FilterChip
                value="unread"
                label={t("employee.notifications.unread")}
                active={filter === "unread"}
                onClick={setFilter}
              />
              {Object.keys(TYPE_STYLES).map((typeKey) => (
                <FilterChip
                  key={typeKey}
                  value={typeKey}
                  label={t(`notificationTypes.${typeKey}`) || typeKey}
            ...
            <section className="rounded-[24px] bg-white border border-[#E5E2DC] overflow-hidden">
              {loading && (
                <p className="px-5 py-10 text-center text-sm text-[#7A8088]">
                  {t("employee.notifications.loading")}
                </p>
              )}
                  ...
                  >
                    <div className="flex justify-between items-start gap-3 mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          {t(`notificationTypes.${n.type}`) || n.type}
                        </span>
                        {!n.is_read && (
                    ...
                    {n.activity_title && (
                      <p className="text-xs text-[#7A8088] mt-2">
                        {t("employee.notifications.about", { title: n.activity_title })}
                      </p>
                    )}
